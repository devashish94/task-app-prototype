...
export default function SidebarView() {
  ...
  return (
    <>
      {/* desktop sidebar */}
      <div className={`z-50 h-screen large:h-full w-full mobile:w-72 2xl:w-80 hidden large:flex 2xl:rounded-l-full duration-300 transition-all transform-gpu ${bottomMenu ?
        'opacity-75' :
        ''
      }`}>
        <Sidebar />
      ...
      {/* tablet sidebar */}
      <div className={`bg-slate-100 z-50 will-change-transform overflow-auto hidden split-display:flex w-0 large:hidden h-screen duration-[300ms] transform-gpu ease-in-out transition-all ${sidebar ?
        'w-96' : 'w-0'}`}>
        <Sidebar />
      </div>
