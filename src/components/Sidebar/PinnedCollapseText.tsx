    ...
    <div className={`flex flex-col mt-2 rounded-lg duration-200 ${visible ? 'bg-slate-200' : 'bg-slate-100'}`}>
      <div className={`font-bold flex justify-between mb-1 px-2 py-2 hover:cursor-pointer rounded-md ${visible ? 'hover:bg-slate-300' : 'hover:bg-slate-200'} `}
        ...
        <div className="flex items-center">
          <div className={`transform-gpu duration-[400ms] ease-in-out transition-all delay-0 ${visible? 'rotate-0' : '-rotate-180'}` }>
            <DownArrow />
          </div>
        </div>
      </div>
      <div className={`overflow-hidden transition-all duration-[400ms] ease-in-out transform-gpu py-0 ${visible ? 'max-h-64' : 'max-h-0'}`}>
